/**
 * stream(): a component-owned async iterable. It connects after the render
 * that declared it commits, publishes each value to the components that read
 * its snapshot, and disconnects with the component.
 */
...
interface StreamSlot<T> {
  ...
  /** Read by components that render the snapshot. */
  readonly readers: ReadableSource;
}
...
function queueStart<T>(slot: StreamSlot<T>): void {
  ...
  queueTask(() => {
    ...
    if (!slot.owner || slot.owner.disposed || slot.explicitlyClosed) {
      return;
    }
    startSlot(slot);
  });
