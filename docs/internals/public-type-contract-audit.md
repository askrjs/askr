...
| Scope              | `@askrjs/askr`                                                             | `tests/types/context.test-d.tsx`                                                                                                                 | `defineScope()`, `readScope()`, scoped value enforcement, child contract, consumer inference                                                                               |
...
- `Scope` accepted arbitrary `unknown` children even though scoped content flows through the normal renderable-child pipeline, and its truthiness checks could drop valid scalar children like `0` and `''`.
...
- The docs snippet compiler's virtual root paths were not normalized on Windows, so root-file-not-found diagnostics could be dropped and some published snippets were not actually being typechecked there. That also let the stale `ThemeScope.provide(...)` docs example slip past the check.
...
- `Scope` also accepted arbitrary `unknown` children even though it only forwards normal renderable content or a zero-argument render callback, and its runtime used truthiness checks that could drop valid scalar children like `0` or `''`.
...
- Updated the stale shared-scope example in `docs/core/data.md` to use the callable `Scope`, matching the real public API.
...
- Tightened `Scope` children to normal renderable content or a zero-argument render callback, matching the runtime's supported branches, and fixed scalar child checks so single `0` and `''` children are preserved instead of being dropped by truthiness.
