...
describe('selector edge cases', () => {
  ...
  // Finding 8 (regression guard): notifySelectorLaneValueChange uses
  // peekCandidateSource (not getCandidateSource) so never-read values do not
  // materialize phantom candidate sources in the lane cache.
  it('should not materialize candidate sources for values that were never read', () => {
    ...
    expect(probe._lane?._primitiveCandidates.has(100)).toBe(false);
    expect(probe._lane?._primitiveCandidates.has(200)).toBe(false);
    expect(probe._lane?._primitiveCandidates.has(300)).toBe(false);
  });
